...
##  -------------
    ...
    hr <- read.csv("../../Courses/Gen Infiniti Academy Pte Ltd/S5/Andy/HR_comma_sep.csv", header=T, na.strings=c('','.','NA'))
...
##  ---------------------
    ...
    ##  (a) Create training and testing set
    ##  -------------------------------------
    ...
    ##  ------------------------------------
    trained_model <- rpart(left~., 
                           data = hr_train, 
                           method = "class")
    
        ## (b.1) predict using trained model on training set
        ##  ------------------------------------------------
          pred.train <- predict(trained_model, 
                                hr_train[,-7], 
                                type = "class")
          
          mean(pred.train == hr_train$left) #[1] 0.9685807

    
          
        ##  (b.2) predict using trained model on testing set
        ##  ----------------------------------
          pred.test <- predict(trained_model, 
                               hr_test[,-7], 
                               type = "class")
      
          mean(pred.test == hr_test$left) #[1] 0.968
