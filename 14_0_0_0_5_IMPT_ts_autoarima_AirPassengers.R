...
##  =============================================
    ...
    class(AirPassengers) # this tells us that data series is in a time seies format
    ...
    AirPassengers #this will print the dataset
    # View(AirPassengers)
...
##  -------------------------------------------------------------------------------  
    ...
    library(tseries)
    adf.test(diff(log(AirPassengers)))  # p-value low --> reject H0 --> stationary
    ...
    Box.test(diff(log(AirPassengers)), lag=20, 
...
##  ------------------------------------------
    ##  Let's split dataset into training set, and hide one year (1960) data for validation  
          datawide <- ts(AirPassengers, frequency = 12, 
                         start=c(1949,1), end=c(1959,12))
          
    ##  Using the model on training set  
          best_model <- Arima(log(datawide), c(0,1,1), 
    ...
    ## Let's predict using trained model  & compare with original data for 1960    
          pred_best_model <- predict(best_model, 
                                     n.ahead = 10*12) # predict next 10 years         
          ...
          predicted_best_1960 <- round(data_best, 
                                       digits = 0)  # rounding off to integer values
          predicted_best_1960 
