...
##  1.  Get Data
##  --------------
  # setwd("C:\\Users\\Andy\\Documents\\GitHub\\Introductory_TimeSeries_R\\Introductory_Time_Series_with_R_datasets") 
  cbe <- read.table("../../Courses/Gen Infiniti Academy Pte Ltd/Andy/Andy_Wiley_DataFiles-20210320T105054Z-001/Andy_Wiley_DataFiles/data_timeSeries/cbe.dat", header=T) 
  head(cbe,2)  
      #   choc beer elec
      # 1 1451 96.3 1497
      # 2 2037 84.4 1463
...
##  ---------------------------------------------  
  ...
  plot(diff(elec.ts))      ## heteroskedasticity
  plot(diff(log(elec.ts))) ## looks like stationary ts, after log and diff transformation
...
##  ------------------------------------------------------------------
  ...
  Box.test(diff(log(elec.ts)), lag=20, 
           type="Ljung-Box") ## p-value low --> reject H0 --> not white noise
...
##  -----------------------------------------------     
  ...
  model1 <- Arima(log(elec.ts), c(0,1,1), 
...
##  -----------------------------------
  ...
  ts.plot(elec.ts, pred_model1_antilog, 
          linear = "y", lty = c(1,2))  # visualise data
