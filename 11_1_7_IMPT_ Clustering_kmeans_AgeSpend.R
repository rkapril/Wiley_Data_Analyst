...
##  --------------------------------
  df <- data.frame(age = c(18, 21, 22, 24, 26, 
                           26, 27, 30, 31, 35, 
                           39, 40, 41, 42, 44, 
                           46, 47, 48, 49, 54),
                   spend = c(10, 11, 22, 15, 12, 
                             13, 14, 33, 39, 37,
                             44, 27, 29, 20, 28, 
                             21, 30, 31, 23, 24)
  )
...
##  --------------------------
  ...
  set.seed(2)
  k2model <- kmeans(df[,c("age", "spend")], 
                    centers=2, nstart=10) ## k = 2
  
  set.seed(1)
  k3model <- kmeans(df[,c("age", "spend")], 
                    centers=3, nstart=10) ## k = 3
  
  set.seed(12)
  k4model <- kmeans(df[,c("age", "spend")], 
                    centers=4, nstart=10) ## k = 4
